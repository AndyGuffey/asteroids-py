...
- `asteroid.py` — `Asteroid`, extends `CircleShape` with its own `draw()`/`update()` overrides, rendering a lumpy polygon outline (via `_outline_points()`) that moves by its velocity, plus `score_value()` returning the points awarded for destroying it based on its size
